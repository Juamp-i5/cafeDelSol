 ...
 *
 * @author pablo
  * Esta clase es un Objeto de Transferencia de Datos (DTO) diseñado específicamente
 * para la creación de nuevas entradas en la capa de persistencia. Su objetivo principal
 * es encapsular los datos necesarios para registrar una nueva entrada sin incluir
 * campos como el ID de la entrada, ya que este suele ser generado automáticamente
 * por la base de datos al momento de la inserción. Sirve como un contrato claro
 * entre la capa de negocio y la capa de acceso a datos para operaciones de registro.
 */
public class EntradaNuevaDTOPersistencia {
    ...
    /**
     * Constructor por defecto. Es esencial para que los frameworks de mapeo de datos
     * puedan instanciar objetos de esta clase, por ejemplo, al deserializar datos.
     */
    public EntradaNuevaDTOPersistencia() {
    }

    /**
     * Constructor completo que permite inicializar todos los atributos de una nueva entrada.
     * Facilita la creación de objetos DTO con la información lista para ser enviada
     * a la capa de persistencia para su registro.
     *
     ...
     * @param DetallesEntrada Una lista de objetos `DetalleEntradaDTOPersistencia` que componen esta entrada.
     */
     ...
     * del objeto `EntradaNuevaDTOPersistencia`. Esto es muy útil para depuración
     * y para registrar la información de las entradas antes de su persistencia.
