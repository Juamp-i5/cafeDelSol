...
/**
 *
 * @author pablo Esta clase funciona como un Objeto de Transferencia de Datos
 * (DTO) para representar una entrada ya existente en el sistema, especialmente
 * diseñada para ser utilizada cuando se recuperan datos de la capa de
 * persistencia. A diferencia de `EntradaNuevaDTOPersistencia`, incluye el
 * identificador único (`idEntrada`), ya que este es un dato fundamental de un
 * registro ya guardado. Sirve para transferir la información completa de una
 * entrada, incluyendo sus detalles, desde la base de datos a otras capas de la
 * aplicación.
 */
public class EntradaViejaDTOPersistencia {
    ...
    /**
     * Constructor por defecto. Es indispensable para que los frameworks de
     * mapeo (ORM) puedan crear instancias de esta clase al deserializar los
     ...
     * entrada existente. Este constructor es útil cuando se construye el DTO
     * con todos los datos necesarios para ser transferidos desde la capa de
     * persistencia.
     *
     ...
     * @param DetallesEntrada Una lista de `DetalleEntradaDTOPersistencia` que
    ...
    /**
     * Constructor alternativo que permite inicializar una
     * `EntradaViejaDTOPersistencia` sin el ID de la entrada, aunque este DTO
     * está más orientado a registros existentes que ya poseen un ID. Puede ser
     * útil en escenarios donde el ID se asigna posteriormente o se obtiene de
     * otra fuente.
     *
     * @param fechaHora La fecha y hora en que se registró la entrada.
     * @param proveedor El nombre del proveedor de la entrada.
    ...
    /**
     * Obtiene el identificador único de la entrada. Este ID es crucial para
     * referenciar y realizar operaciones sobre una entrada ya existente en la
     * base de datos.
     *
     ...
     * legible del objeto `EntradaViejaDTOPersistencia`. Esto es muy útil para
     * depuración y para registrar la información de las entradas después de ser
