 ...
 *
 * @author pablo
* Esta clase `DetalleEntradaMapperPersistencia` actúa como un **mapeador** entre
 * la entidad `DetalleEntrada` (el modelo de dominio) y el DTO
 * `DetalleEntradaDTOPersistencia` (el objeto de transferencia de datos para persistencia).
 * Su función principal es traducir datos de un formato a otro, asegurando que la capa
 * de persistencia trabaje con los DTOs, mientras que la capa de negocio manipula las entidades.
 * Implementa el patrón Singleton para asegurar una única instancia del mapeador.
 */
public class DetalleEntradaMapperPersistencia implements IDetalleEntradaMapperPersistencia {
    ...
    /**
     * Implementa el patrón Singleton para proporcionar una única instancia de este mapeador.
     * Si la instancia aún no ha sido creada, la inicializa y la devuelve; de lo contrario,
     * retorna la instancia existente. Esto es útil para optimizar recursos y asegurar
     * que todos los componentes utilicen el mismo objeto de mapeo.
     *
    ...
    /**
     * Convierte un objeto `DetalleEntradaDTOPersistencia` (DTO de la capa de persistencia)
     * a su correspondiente entidad de dominio `DetalleEntrada`. Este método se encarga
     * de transferir los datos del DTO a la entidad, incluyendo la conversión del ID del
     * ingrediente de `String` a `ObjectId`, que es el tipo utilizado en la entidad para
     ...
     * `DetalleEntradaDTOPersistencia`. Este método es crucial para preparar los datos
     * de las entidades para ser transferidos fuera de la capa de acceso a datos,
     * por ejemplo, a la capa de negocio o presentación. Incluye la conversión del
     * `ObjectId` del ingrediente a `String` y el mapeo de la información del ingrediente anidada.
     *
    ...
    /**
     * Convierte una lista de objetos `DetalleEntradaDTOPersistencia` a una lista de entidades `DetalleEntrada`.
     * Itera sobre la lista de DTOs, aplicando el método `toEntity` a cada elemento
     * para realizar la conversión individual y construir la lista de entidades resultante.
     *
    ...
    /**
     * Convierte una lista de entidades `DetalleEntrada` a una lista de objetos `DetalleEntradaDTOPersistencia`.
     * Este método es vital para la transferencia de colecciones de datos, asegurando que
     * cada entidad se transforme correctamente en su formato DTO antes de ser transferida.
     ...
     * @return Una `List` de objetos `DetalleEntradaDTOPersistencia`. Retorna una lista vacía si la entrada es `null`.
     */
    @Override
